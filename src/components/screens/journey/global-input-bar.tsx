...
export function GlobalInputBar({
  ...
  const activeMode = activeTab === "timeline" ? "note" : "ask";
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 px-6 pb-[max(env(safe-area-inset-bottom),40px)] pt-3">
      <div className="mx-auto max-w-[430px]">
          ...
          {(keyboardOpen || text) && (
            ...
            >
              <textarea
                ...
                className="journey-liquid-glass mb-3 w-full resize-none rounded-[24px] px-4 py-3 text-base font-medium leading-6 text-slate-700 outline-none placeholder:text-slate-400"
              />
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {isRecording && (
          <div className="journey-liquid-glass mb-3 rounded-[22px] px-4 py-3 text-sm font-semibold text-slate-600">
            {liveTranscript || "正在听你说话..."}
          </div>
        )}

        <div className="journey-liquid-glass flex items-center gap-3 rounded-[32px] px-4 py-3">
          <button
          ...
          >
            {text.trim() ? (
              <span className="journey-glass-card inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold text-slate-700">
